                                          ...
                                          const int *row_ptr, const int *col_ind, const int *diag_ind, double *nnz_data)
{
    ...
    for (idx=0; idx<neles; idx++) {
        ...
        for (kdx=st; kdx<ed; kdx++) {
            ...
            // 1) mat := A[k,k]
            cblas_dcopy(nvars*nvars, &nnz_data[ldx*nvars*nvars], 1, mat, 1);
            // 2) Solve A[i,k] = inv(A[k,k]) @ A[i,k]
            info = LAPACKE_dgetrf(LAPACK_ROW_MAJOR, nvars, nvars, mat, nvars, ipiv);
            if (info != 0) printf("Lapack dgetrf function error\n");
            info = LAPACKE_dgetrs(LAPACK_ROW_MAJOR, 'N', nvars, nvars, mat, nvars, ipiv, &nnz_data[kdx*nvars*nvars], nvars);
            if (info != 0) printf("Lapack dgetrf function error\n");
            // 3) mat := A[i,k]
            cblas_dcopy(nvars*nvars, &nnz_data[kdx*nvars*nvars], 1, mat, 1);
